...
def parse_consonant(phon):
    ...
    # Some affricates in PHOIBLE have a length diacritic between the glyphs.
    if 'ːː' in phon:
        parse['length'] = 'overlong'
        length_idx = phon.find('ːː')
        phon = phon[:length_idx] + phon[length_idx+2:]
    elif 'ː' in phon:
        parse['length'] = 'long'
        length_idx = phon.find('ː')
        phon = phon[:length_idx] + phon[length_idx+1:]
