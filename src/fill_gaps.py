...
def places_congruent(p1, p2):
    fd = ql.feature_difference(p1, p2)
    if len(fd) == 1 and 'place' in fd and \
        (
            sorted([fd['place']['p1'], fd['place']['p2']]) == ['alveolar', 'dental'] 
            or
            sorted([fd['place']['p1'], fd['place']['p2']]) == ['bilabial', 'labio-dental']
        ):
        return True
    else:
        return False
...
def different(a, b, c, x):
    ...
    if ql.feature_difference(a, x) == {} or ql.feature_difference(b, x) == {} or ql.feature_difference(c, x) == {}:
        return False
    if places_congruent(a, x) or places_congruent(b, x) or places_congruent(c, x):
        return False
    return True
...
if __name__ == '__main__':
    ...
    for key in gaps:
        ...
        for d in filter(lambda x: different(a, b, c, x), reference_segments):
            ...
            result[key] = None
    with open('../json/affricate_gaps_fillers.json', 'w', encoding='utf-8') as out:
